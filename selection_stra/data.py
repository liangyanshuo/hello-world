...
class XYDataset(torch.utils.data.Dataset):
    def __init__(self, x, y, **kwargs):
        ...
        self.indices = torch.arange(len(self.x)).long()
    ...
    def __getitem__(self, idx):
        x, y, id = self.x[idx], self.y[idx], self.indices[idx]
...
def get_mnist(args):
    ...
    # fetch MNIST
    train = datasets.MNIST(args.data_path, train=True,  download=True)
    test  = datasets.MNIST(args.data_path, train=False, download=True)
    ...
    train_ds = XYDataset(train_ds[0][0], train_ds[0][1], **{'source':'mnist'})
    val_ds = XYDataset(val_ds[0][0], val_ds[0][1], **{'source':'mnist'})
    test_ds = XYDataset(test_ds[0][0], test_ds[0][1], **{'source':'mnist'})
